...
La réponse finale du modèle doit être un JSON strict `{"status": "answered | insufficient_evidence | out_of_scope | refused", "answer": "...", "used_chunk_ids": [...]}` (palier 5 — le champ `status` s'ajoute au contrat du palier 3 ; `out_of_scope` distingue une demande hors documents, sans aucun appel d'outil, d'une question corpus sans preuve) — les citations sont validées après coup contre les `chunk_id` réellement retournés par les appels d'outils du run (`runtime.returned_chunk_ids`), pas simplement recopiées depuis ce que dit le modèle. Le `status` lui-même n'est pas non plus pris tel quel : un `answered` sans preuve valide est rétrogradé en `insufficient_evidence` par le code, et le texte libre est ignoré pour `insufficient_evidence`, `out_of_scope` et `refused` (messages serveur fixes) — voir `grounding_confidence` en section 5, et AGENTS.md section 9 pour le garde-fou qui peut le rétrograder.
