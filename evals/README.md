...
- `httpx.AsyncClient.post` est monkeypatché (scénario `provider_unavailable`) pour simuler une vraie panne réseau à travers le vrai chemin `_real_anthropic_async` (moteur async), pas un double de cette fonction.
...
Rejoue quelques scénarios (`factual`, `absurd`, `hostile`, `no-result`) contre une vraie instance avec un vrai appel Anthropic — pas exécuté en CI, affiche tokens et coût, n'enregistre jamais de clé. État au gel : **non implémenté** — les vérifications live sont faites manuellement (curl + serveur local), cf. DURCISSEMENT.md.
