...
Le risque central du palier 5 : un agent qui répond avec la même assurance qu'il sache ou qu'il invente. Contrat pas encore livré côté backend au moment de la rédaction (`status`, `confidence`, `metrics`) — voir DURCISSEMENT.md.
...
| T22 | Réponse confiante sur une question sans rapport avec le corpus | Pas de statut dédié pour l'instant ; le comportement actuel n'invente pas de contenu (le modèle reste contraint aux passages fournis) mais ne signale pas explicitement "je ne sais pas" à l'utilisateur | Le champ `status="insufficient_evidence"` doit rendre ça explicite plutôt qu'implicite (contrat palier 5, pas livré) |
| T23 | Score du détecteur d'injection confondu avec une probabilité de vérité de la réponse | Renommé "Score détecteur" partout dans l'interface (palier 5), distinct de la confiance de grounding de la réponse | Le champ `confidence` de la réponse elle-même n'existe pas encore côté backend |
| T24 | Coût caché à l'utilisateur (connu seulement des logs) | Frontend prêt à afficher tokens/durée/coût dès que `metrics` existe, jamais arrondi à $0 si le coût réel est non nul | `metrics.estimated_cost_usd` n'existe pas encore côté backend au moment de la rédaction |
