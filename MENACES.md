...
Le risque central du palier 5, étendu au périmètre conversationnel : un agent qui répond avec la même assurance qu'il sache, qu'il ignore ou qu'on lui demande autre chose que son travail. Contrat livré côté backend par Erwan (`status` — quatre valeurs : `answered`, `insufficient_evidence`, `out_of_scope`, `refused` — `confidence`, `metrics` sur `Answer` — `backend/models.py`, `backend/agent.py`, `backend/main.py`, `backend/metrics.py`) et testé en direct contre le vrai modèle — voir DURCISSEMENT.md.

| # | Menace | Mitigation réelle | Limite réelle |
|---|---|---|---|
| T21 | Citation inventée par le modèle (chunk_id qui n'a jamais été retourné par un outil) | Filtrée avant d'atteindre l'utilisateur : `_ground_final_answer` ne garde que les `chunk_id` réellement dans `runtime.returned_chunk_ids`, et un `answered` sans citation valide est rétrogradé en `insufficient_evidence` | Testé et confirmé (`evals/run_eval.py` scénario `invented_citation_filtered`) — risque résiduel : une affirmation inventée rattachée à des citations valides mais sémantiquement sans rapport (le garde-fou vérifie la provenance des citations, pas la pertinence sémantique phrase à phrase) |
| T22 | Question hors rapport avec le corpus (« As-tu joué à Mario ? », « Quelle est la capitale du Japon ? ») | `status="out_of_scope"` : aucun appel d'outil, aucune connaissance générale, message serveur fixe. À ne pas confondre avec `insufficient_evidence` (question corpus, recherche effectuée, preuve absente — ex. « Qui possède COBOL ? ») | Testé en mocké (`tests/test_scope.py`) et en direct : 0 `tool_call` au journal, confiance `n/a`. La classification `corpus`/`out_of_scope` appartient au modèle (aucun routage lexical dans le code) — un modèle qui classerait mal reste borné par les messages serveur et l'absence de citations |
